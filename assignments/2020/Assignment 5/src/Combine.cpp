...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    if (sequences.size() == 0) {
        ...
        Vector<DataPoint> left = combine(sequences.subList(0, mid));
        Vector<DataPoint> right = combine(sequences.subList(mid, sequences.size() - mid));
        Vector<DataPoint> combine;

        int i1 = 0, i2 = 0;
        for (int i = 0; i < left.size() + right.size(); i++) {
            if (i2 >= right.size() || (i1 < left.size() && left[i1].weight < right[i2].weight)) {
                combine.add(left[i1]);
                i1++;
            } else {
                combine.add(right[i2]);
                i2++;
            }
        }
        cout << combine << endl;
...
ostream& operator <<(ostream& out, Vector<DataPoint> sequences) {
    out << "{";
    for (DataPoint dp: sequences) {
        out << "name: " << dp.name << " weight: "<< dp.weight << "|";
    }
    out << "{";
    return out;
}
