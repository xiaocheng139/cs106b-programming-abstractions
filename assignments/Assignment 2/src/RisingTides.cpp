...
Vector<GridLocation> generateNeighbors(const Grid<double>& terrain, GridLocation location);
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> results(terrain.numRows(), terrain.numCols());

    for (GridLocation source: sources) {
        queue.enqueue(source);
        results[source.row][source.col] = true;
    }

    while (!queue.isEmpty()) {
        GridLocation cur = queue.dequeue();
        Vector<GridLocation> neighbors = generateNeighbors(terrain, cur);

        for (GridLocation neighbor: neighbors) {
            if (terrain[neighbor.row][neighbor.col] <= height && results[neighbor.row][neighbor.col] == false) {
                results[neighbor.row][neighbor.col] = true;
...
Vector<GridLocation> generateNeighbors(const Grid<double>& terrain, GridLocation location) {
    ...
    if (terrain.inBounds(location.row, location.col - 1)) {
        GridLocation leftLocation = {location.row, location.col - 1};
        neighbors.add(leftLocation);
    }

    if (terrain.inBounds(location.row, location.col + 1)) {
        GridLocation rightLocation = {location.row, location.col + 1};
        neighbors.add(rightLocation);

    }

    if (terrain.inBounds(location.row + 1, location.col)) {
        GridLocation upLocation = {location.row + 1, location.col};
        neighbors.add(upLocation);

    }

    if (terrain.inBounds(location.row - 1, location.col)) {
        GridLocation downLocation = {location.row - 1, location.col};
        neighbors.add(downLocation);
