...
bool canBeMadeDisasterReadyHelper(const HashMap<string, HashSet<string>>& roadNetwork, int numCities, HashSet<string>& uncoverred, HashSet<string>& supplyLocations);
...
bool canBeMadeDisasterReady(const HashMap<string, HashSet<string>>& roadNetwork,
    ...
    if (numCities < 0) {
        error("Number of cities should not be negative!");
    }
    Vector<string> uncoverredLocations = roadNetwork.keys();
    HashSet<string> uncoverred;
    HashSet<string> affected;
    for (string location: uncoverredLocations) {
        uncoverred.add(location);
    }
    bool result = canBeMadeDisasterReadyHelper(roadNetwork, numCities, uncoverred, supplyLocations);
    return result;
}
...
bool canBeMadeDisasterReadyHelper(const HashMap<string, HashSet<string>>& roadNetwork, int numCities, HashSet<string>& uncoverred, HashSet<string>& supplyLocations) {
    ...
    if (uncoverred.isEmpty()) {
        return true;
    }

    string location = uncoverred.front();
    HashSet<string> affected = roadNetwork[location] + location;
    HashSet<string> newUncoverred;

    for (string location: affected) {
        newUncoverred = uncoverred;
        newUncoverred -= location;
        for(string s: roadNetwork[location]) {
            if (uncoverred.contains(s)) {
                newUncoverred -= s;
            }
        }
        supplyLocations.add(location);
        if (canBeMadeDisasterReadyHelper(roadNetwork, numCities - 1, newUncoverred, supplyLocations)) {
            return true;
        }
        supplyLocations.remove(location);
    }
